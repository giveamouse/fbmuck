...
propdir_first_elem(PropPtr root, char *path)
{
    ...
    char *n;
    while (*path && *path == PROPDIR_DELIMITER) path++;
    if (!*path) return(first_node(root));
...
propdir_name(const char *name)
{
    ...
    o = pnbuf;
    while (*i) {
	while (*i == PROPDIR_DELIMITER) i++;
	*o++ = PROPDIR_DELIMITER;
	while (*i && *i != PROPDIR_DELIMITER) *o++ = *i++;
    }
    *o = '\0';
    while (*o != PROPDIR_DELIMITER) *o-- = '\0';
    return pnbuf;
}
...
propdir_unloaded(PropPtr root, const char *path)
{
    ...
    n = path;
    o = buf;
    while (*n == PROPDIR_DELIMITER) n++;
    if (!*n) return NULL;
    while (*n) {
	*o++ = '/';
	l = o;
	while (*n && *n != PROPDIR_DELIMITER) *o++ = *n++;
	while (*n == PROPDIR_DELIMITER) n++;
	*o = '\0';
