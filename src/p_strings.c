    ...
    if (sstr[scnt]==FMTTOKEN) {
      if (sstr[scnt+1]==FMTTOKEN) {
        ...
        if ((sstr[scnt]>='0')&&(sstr[scnt]<='9')) {
          while ((sstr[scnt]>='0')&&(sstr[scnt]<='9')) scnt++;
        } else {
          if (sstr[scnt]=='*') {
            scnt++;
            CHECKOP(1);
        ...
        sfmt[0]='%';
        sfmt[1]='\0';
        if (slrj==1) strcat(sfmt,"-");
        if (spad1)
          if (spad1==1)
            strcat(sfmt,"+");
          else
            strcat(sfmt," ");
        if (spad2) strcat(sfmt,"0");
        if (slen1!=-1) {
        ...
        switch(sstr[scnt]) {
          case 'i':
            ...
            if (slrj==2) {
              tnum=0;
              while ((tbuf[tnum]==' ')&&(tnum<tlen)) tnum++;
              if ((tnum)&&(tnum<tlen)) {
                temp=tnum/2;
                for (i=tnum; i<tlen; i++)
                  tbuf[i-temp]=tbuf[i];
...
prim_split(PRIM_PROTOTYPE)
{
 ...
 oper1 = POP();
 oper2 = POP();
 if (oper1->type!=PROG_STRING) abort_interp("Non-string argument. (2)");
 if (!oper1->data.string) abort_interp("Null split argument. (2)");
 if (oper2->type!=PROG_STRING) abort_interp("Non-string argument. (1)");
...
prim_itoc(PRIM_PROTOTYPE)
{
 ...
 if (oper1->data.number > 127 || (!isprint((char)oper1->data.number) &&
   ((char)oper1->data.number!='\r') && ((char)oper1->data.number!='\[')))
 {
  result=0;
 } else {
  result=1;
...
prim_stod(PRIM_PROTOTYPE)
{
 ...
 if (!oper1->data.string) {
  ...
  tmp=0;
  while(oper1->data.string->data[tmp]==' ') tmp++;
