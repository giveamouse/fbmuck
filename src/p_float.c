...
prim_float(PRIM_PROTOTYPE)
{
 CHECKOP(1);
 oper1 = POP();
 if (oper1->type != PROG_INTEGER) abort_interp("Non-integer argument. (1)");
...
prim_pow(PRIM_PROTOTYPE)
{
 ...
 oper1 = POP();
 oper2 = POP();
 if (oper2->type != PROG_FLOAT) abort_interp("Non-float argument. (1)");
 if (oper1->type != PROG_FLOAT) abort_interp("Non-float argument. (2)");
...
prim_strtof(PRIM_PROTOTYPE)
{
 CHECKOP(1);
 oper1 = POP();
 if (oper1->type != PROG_STRING) abort_interp("Non-string argument. (1)");
 fresult = 0.0;
