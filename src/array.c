...
{
    if (a->type != b->type) {
        if (a->type == PROG_STRING || b->type == PROG_STRING) {
            return (a->type - b->type);
        }
        if (a->type == PROG_INTEGER) {
...
array_tree_delete(array_iter* key, array_tree* avl)
{
    ...
    if (save) array_tree_free_node(save);
    return avl;
}
...
array_tree_prev_node(array_tree* ptr, array_iter *key)
{
    ...
    if (cmpval > 0) {
        ...
        from = AVL_LF(ptr);
        while (AVL_RT(from)) from = AVL_RT(from);
        return from;
    } else {
        return NULL;
    }
    ...
    switch (arr->type) {
        case ARRAY_PACKED: {
            ...
            if (idx->data.number >= 0 && idx->data.number < arr->items) {
                ...
                arr->data.packed = realloc(arr->data.packed, sizeof(array_data) * (arr->items + 1));
...
array_insertrange(stk_array** harr, array_iter* start, stk_array* inarr)
{
    ...
    switch (arr->type) {
        case ARRAY_PACKED: {
            ...
            arr->data.packed = realloc(arr->data.packed, sizeof(array_data) * (arr->items + inarr->items));
            copyinst(start, &idx);
            copyinst(start, &didx);
...
array_demote_only( stk_array* arr, int threshold )
{
  ...
  if (!arr || ARRAY_DICTIONARY != arr->type) return NULL;
...
array_mash( stk_array* arr_in, stk_array **mash, int value ) {
  ...
  if (NULL == arr_in || NULL == mash || NULL == *mash) return ;
