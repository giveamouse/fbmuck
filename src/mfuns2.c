...
dbref   obj;
    ...
    if (obj == HOME) obj = PLAYER_HOME(player);
    if (argc > 1) {
	...
	if (obj2 == HOME) obj2 = PLAYER_HOME(player);
    ...
    *buf = '\0';
    ...
    while (*ptr) {
        ...
        if (!*p) {
            ...
            if (outlen + nextlen > BUFFER_LEN - 3)
                break;
            if (outcount++) strcat((buf+(outlen++)), "\r");
