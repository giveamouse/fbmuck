...
/*
 * $Log: move.c,v $
 * Revision 1.1  2000/01/14 22:56:07  revar
 * Initial revision
 *
...
parent_loop_check(dbref source, dbref dest)
{
    ...
    if (Typeof(dest) == TYPE_THING &&
            parent_loop_check(source, THING_HOME(dest)))
        return 1;
...
void enter_room(int descr, dbref player, dbref loc, dbref exit)
{
    ...
    if (tp_penny_rate != 0) {
        ...
        if (!controls(player, loc)
                && PLAYER_PENNIES(player)<= tp_max_pennies
                ...
                if (dest == getloc(exit) && (FLAGS(dest) & VEHICLE)) {
                    ...
                    if (Typeof(DBFETCH(exit)->location) == TYPE_THING) {
                        ...
                        if (tp_thing_movement) {
                          enter_room(descr,dest,DBFETCH(DBFETCH(exit)->location)->location,exit);
                        } else {
    ...
    switch (Typeof(thing)) {
        ...
        case TYPE_PROGRAM:
            ...
            if (Typeof(cont) == TYPE_ROOM && (FLAGS(thing) & STICKY) &&
                ...
                if (tp_thing_movement&&(Typeof(thing) == TYPE_THING)) {
                  enter_room(descr,thing,immediate_dropto ? DBFETCH(cont)->sp.room.dropto : cont,player);
                } else {
