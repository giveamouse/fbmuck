...
/*
 * $Log: look.c,v $
 * Revision 1.1  2000/01/11 01:58:16  revar
 * Initial revision
 *
...
flag_description(dbref thing)
{
    ...
    if (FLAGS(thing) & ~TYPE_MASK) {
        ...
        if (FLAGS(thing) & ZOMBIE)
            strcat(buf, " ZOMBIE");
        if (FLAGS(thing) & HAVEN)
            strcat(buf, (Typeof(thing) != TYPE_PROGRAM) ? ((Typeof(thing) == TYPE_THING)? " HIDE" : " HAVEN") : " HARDUID");
...
do_examine(int descr, dbref player, const char *name, const char *dir)
{
    ...
    switch (Typeof(thing)) {
        ...
        case TYPE_THING:
            /* print home */
            sprintf(buf, "Home: %s",
                    unparse_object(player, THING_HOME(thing)));        /* home */
            notify(player, buf);
        ...
        case TYPE_PLAYER:
            ...
            /* print home */
            sprintf(buf, "Home: %s",
                    unparse_object(player, PLAYER_HOME(thing)));        /* home */
            notify(player, buf);
