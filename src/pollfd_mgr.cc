...
struct pollfd *pollfd_mgr_add(struct pollfd_mgr *mgr, int fd, short event) {
    ...
    return &(mgr->store[pos]);
}

struct pollfd *pollfd_mgr_remove(struct pollfd_mgr *mgr, int fd, short event) {
    std::map<int, int>::iterator iter = mgr->posMap.find(fd);
    if (iter != mgr->posMap.end()) {
...
struct pollfd *pollfd_mgr_get(struct pollfd_mgr *mgr, int fd) {
    return pollfd_mgr_add(mgr, fd, 0);
}
...
short pollfd_mgr_result(struct pollfd_mgr *mgr, int fd) {
    std::map<int, int>::iterator iter = mgr->posMap.find(fd);
    if (iter == mgr->posMap.end()) {
        return 0;
    }
    return mgr->store[iter->second].revents;
}
